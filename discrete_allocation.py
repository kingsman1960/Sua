...
class DiscreteAllocation:
    ...
    def __init__(
        ...
        if not isinstance(weights, dict):
            raise TypeError(
                "weights should be a dictionary of {ticker: weight}")
        ...
        for ticker, weight in self.weights:
            if ticker in self.allocation:
                allocation_weight = (
                    ...
                    self.latest_prices[ticker] / portfolio_val
                )
            else:
                allocation_weight = 0
    ...
    def greedy_portfolio(self, reinvest=False, verbose=False):
        ...
        if self.weights[-1][1] < 0:
            ...
            da1 = DiscreteAllocation(
                longs, self.latest_prices[longs.keys(
        ...
        self.allocation = self._remove_zero_positions(
            collections.OrderedDict(
                zip([i[0] for i in self.weights], shares_bought))
        )
